...
class CloudServices extends Component {
  ...
  componentDidMount() {
    ...
    elementResizeEvent(resizeElement, function() {
      self.setState({
        repeatCount: Math.ceil(
          resizeElement.offsetWidth / self.state.repeatWidth
        )
      });
    });
    const timer = setInterval(function() {
      ...
      diff -= 1;
      if (diff < -self.state.repeatWidth) diff += self.state.repeatWidth;
      if (self.state.timer) {
        self.setState({ diff });
      }
    }, 10);
  ...
  componentWillUnmount() {
    var resizeElement = document.getElementById("services-container");
    elementResizeEvent.unbind(resizeElement);
  ...
  render() {
    ...
    for (var i = 0; i < this.state.repeatCount + 1; i++) {
      for (var j = 0; j < this.state.services.length; j++) {
        var style = {
          left:
            this.state.repeatWidth * i +
            this.state.services[j].pos.left +
            this.state.diff,
        ...
        containers.push(
          <CloudService
            key={i * this.state.services.length + j}
            style={style}
            image={this.state.services[j].image}
            label={this.state.services[j].label}
          />
        );
      }
    }
    return (
      <div className={`CloudServices`} style={{ ...this.props.style }}>
        <div
